...
//database bilan ishlayotganimiz uchun async function ishlatayapmiz ekan
...
memberController.getMemberDetail = async (req: ExtendedRequest, res: Response) => {
  try {
      ...
      const result = await memberService.getMemberDetail(req.member);  //bu mantiq orqali userni Datalarini olar ekanmiz
    res.status(HttpCode.OK).json(result);
    } catch (err) {
